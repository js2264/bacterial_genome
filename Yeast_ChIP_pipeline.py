...
def load_clean_bw(filename, strand):
    ...
    return {k: v * 1_000_000 / total for k, v in signals.items() if k != "Mito"}
...
def clean_ratio(ips, ctrls, func="ratio"):
    ...
    if func == "ratio":
        ...
        def func(x, y):
            return np.sqrt(x) * np.log(x / y)
    else:
        assert callable(func)
    return {
        k: np.where((ctrls[k] == 0) | (ips[k] == 0), 0, func(ips[k], ctrls[k]))
        for k in ips.keys()
    }


def invalid_log_ratio(ips, ctrls):
    return {k: np.where((ctrls[k] == 0) | (ips[k] == 0))[0] for k in ips.keys()}
...
if __name__ == "__main__":
    # Load data
    myco_coh_ip = load_clean_bw(Path(data_dir, "GSM6703630_Scc1_Mmyco_rep1.CPM.bw"), "W303")
    ...
    myco_pol_ip_merge = {
        k: (myco_pol_ip[k] + myco_pol_ip2[k]) / 2 for k in myco_pol_ctrl.keys()
    }
    myco_ctrl_merge = {
        k: (myco_coh_ctrl[k] + myco_pol_ctrl[k] + myco_pol_ctrl2[k]) / 3
        for k in myco_coh_ctrl.keys()
    }
    pneu_ctrl_merge = {
        k: (pneu_coh_ctrl[k] + pneu_pol_ctrl[k]) / 2 for k in pneu_coh_ctrl.keys()
    }

    myco_coh = clipnorm(clean_ratio(myco_coh_ip, myco_ctrl_merge, func="ratio"))
    myco_pol = clipnorm(clean_ratio(myco_pol_ip_merge, myco_ctrl_merge, func="ratio"))
